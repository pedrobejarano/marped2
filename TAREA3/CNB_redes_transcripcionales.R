############################################
## Construccion y Analisis de             ##
## Redes Transcripcionales.               ## 
##                                        ##
## Autor: Mario Ruiz y Pedro Bejarano     ##
## Email: fran@us.es                      ##
## Fecha: Febrero 2018                    ##
############################################
...
tfs.data <- read.table(file = "target_gen.txt",as.is=TRUE)
tfs.data
tfs <- tfs.data[,1]
tfs
length(tfs)
...
##Hay que esperar 5 +- 2, está muy lejos de 14 por lo que la autoregulación si es un motivo de nuestra red.
...
plot.igraph(feedback.loop.with.output)
